...
### Error Calculation: 
The loop compares the `target_x` against the known center of the camera feed (200 pixels) to find the "error" (how far off-center the target is).

## State Machine & Proportional Control:

### Scanning: 
If the target is 0, the servo gets a steady 1450 microsecond pulse to slowly pan the room.

### Deadzone (Locked): 
If the error is less than 25 pixels, the target is considered "centered." The servo is sent a 1428 microsecond pulse (the stop command) to prevent jittering.

### Tracking (Left/Right): 
If the target moves outside the deadzone, the Arduino uses the map() function to scale the servo speed proportionally. The further away the target is from the center, the faster the motor spins to catch up. As the target gets closer to the center, the motor slows down for a smooth approach.